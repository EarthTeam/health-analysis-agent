...
export function computeDayAssessment(
    ...
    const morphStress = voteResults.find(v => v.device === "Morpheus" && v.state === "stressed");
    const otherOk = voteResults.filter(v => v.device !== "Morpheus" && v.state === "ok").length >= 2;
    const isAmAutonomicLag = !!(morphStress && otherOk);

    let disagreement = (majority === "mixed") ? true : (voteResults.some((v) => v.state !== "neutral" && v.state !== majority));

    // Feature: AM Autonomic Lag. If ONLY Morpheus is stressed but Oura+Whoop are OK, 
    // it's a wake-up snapshot capturing lag, not a systemic failure.
    if (isAmAutonomicLag) {
        disagreement = false;
    }
    ...
    if (isAmAutonomicLag) {
        why.push("Morning autonomic lag detected: wake-state snapshot shows dip, but overnight recovery (Oura/Whoop) is intact.");
    }
    ...
    const isConsolidating = (isAmAutonomicLag || loadStackFlag || loadMemory > 0.3);

    if (rec === "Green") {
        if (isConsolidating) {
            recText = `Consolidate Rhythm — Stay Rhythmic`;
            plan.push("Maintain rhythm: walk naturally; avoid adding intensity or 'testing' today.");
            plan.push("Trust the absorption process; let the system finish integrating yesterday's load.");
        } else {
    ...
    if (hasDiurnalRebound && stellarRecovery && lowLoad) {
        ...
        mantra = "Integrate & Consolidate: Capacity is present, absorption is ongoing. Maintain rhythm without adding load.";
        scoutCheck = "Notice if the morning lag resolves. Don't test the system; just assist clearance.";
    } else if (majority === "ok" && conf >= 85 && loadMemory < 0.2) {
        mantra = "Expand & Durability: System is harmonized and stable. Maintain rhythm.";
        scoutCheck = "Confirm fluidity through the hips and joints. Keep the 'Wolf' mindset.";
    } else {
        mantra = "Load absorbing overnight. Observe morning lag resolving late morning. Use movement to assist clearance";
    ...
    } else if (loadMemory > 0.3 || signalTension || majority === "mixed" || majority === "stressed" || cycleLabel || isAmAutonomicLag || loadStackFlag) {
